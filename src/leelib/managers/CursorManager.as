package leelib.managers
{
	...
	public class CursorManager
	{
		...
		public function addCursor($id:String, $graphic:DisplayObject, $offsetX:Number=NaN, $offsetY:Number=NaN, $disablesMouse:Boolean=false, $fadesIn:Boolean=false, $rotates:Boolean=false):void
		{
			_graphics[$id] = $graphic;
			_offsetX[$id] = isNaN($offsetX) ? $graphic.width/-2 : $offsetX ;
			_offsetY[$id] = isNaN($offsetY) ? $graphic.height/-2 : $offsetY;
		...
		public function showCursor($id:String):void
		{
			...
			var dob:DisplayObject = _graphics[_id];
			dob.x = _offsetX[_id];
			dob.y = _offsetY[_id];
			_cursorHolder.addChild(dob);
